...
* Run command: `az deployment group create --resource-group Cascadia --template-file main.bicep`
...
1. Reference Cascadia App source code file via GitHub (.env.teamsfx.dev)
   ...
   * REACT_APP_API_ENDPOINT= 
   1. Set View Name = to Requestor or Provider, depending on your status
   2. Set Partner Name = to match how your organization is listed in the database
   3. Set Connection String = to SaaS Token for Data Lake Storage (only applies to requestors)
   4. Set API Endpoint = to match the Endpoint URL that results from GitHub actions
2. Reference Cascadia App source code file via GitHub (config.dev.json)
   1. Reference line "reportendpoint" : "<PowerBI URI>"
   2. Set <PowerBI URI> = (Get this URI from your PowerBI administrator and reference the section at the end of this document on implementing the PowerBI service).
...
4. Change the data source settings of the report to point to your local SQL server by following these steps (Note that this requires that you have ‘read permission’ first from the Cascadia Server):
   1. Select ‘Home’ from menu at the top-left of the screen
   2. From the ‘Home’ ribbon that displays, click ‘Transform data’, then choose ‘Data source settings’ from the drop-down menu.
   3. From the ‘Data source settings’ pop-up window, click the ‘Change Source’ button at the bottom-left.
   4. A pop-up titled ‘SQL Server database’ will appear.  Populate the two fields that are visible (‘Server’ and ‘Database’) with your local Cascadia SQL Server information.  Note that you will have already configured this information as part of installing the Cascadia environment.
   5. Click ‘Ok’ after populating the aforementioned fields.
   6. From the ‘Data source settings’ pop-up, choose ‘Edit Permissions’.
   7. From the ‘Edit Permissions’ pop-up, click the ‘Edit’ button displayed under ‘Credentials’.
   8. From the SQL Server database pop-up, choose the ‘Windows’ tab on the left, then select ‘Use my current credentials’ and click ‘Save’
   9. In the Edit Permissions pop-up, click ‘Ok’
   10. From the ‘Data source settings’ pop-up, click ‘close’
...
From a technical perspective, adding additional tables to the SQL storage that is deployed as part of the infrastructure would require T-SQL experience to provide either CREATE TABLE or ALTER Table statements and a tool like SQL Management Studio to execute this commands.

## API For Data Movement into SQL
In the current implementation both bulk loads of data and individual record alterations are accomplished through the UI calling the APIs in this solution.  As the number of tables was limited, the bulk upload logic was only targeting the Item_Request table.  The individual record modification is being done to the metadata table that contains the data to be pushed to the event hub for messaging.  

The technical requirements to alter this code would be experience with API programming and a coding environment in which to work, like Visual Studio Code.  

To extend this to more tables, there are a couple of changes that need to be made.  Firstly, for the bulk upload, the logic would need to be altered to account for either different tables per upload, or it would need to be modified to split the incoming data across tables.  This change would be largely dependent on the source file and destination in SQL for the new business case.

The other change that would be required would be for the individual record change logic.  Currently this solution uses a metadata table called Notifications to track the record hash and the change value for any updates.  As more tables are included in this solution, an additional column would likely be required in this table to track the tableID as well as record identifier and value.

## Azure Functions For Data Movement Across Tenants
In the current architecture, we are using Azure functions to facilitate the reading and writing of the Event Hub to each “partner” tenant.  

In a similar way that the APIs would need to be modified, the Azure functions would need to be modified.  They would need to be altered to use the tableID column of the notifications table, as well as be modified for bulk uploads to account for the possibility of either different destination tables or splitting the data across tables.

The technical skill set for making this change would be a little different than the API work.  This requires knowledge of Azure as well as experience programming Azure functions.

## User Interface
While the UI in this project is presented to the users in Microsoft Teams, that is not a requirement.  The UI is a web app that was simply embedded in a local organization Teams channel.  This was done to allow each organization to control security and to increase the ability of each organization to disperse access of the tool to members of their team that needed it.  

From a technical perspective, extending this part of the solution would require web development skills.  All of the interactions with data are either reads from local SQL or a data push/modification through API.
